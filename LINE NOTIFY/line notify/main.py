    ...
    try:
        ...
        if location_data:
            ...
            try:
                ...
                if rain_prob_int > 30 and max_temp_int > 30:
                    notification_message = "\n☔ 機率超過30%，🔥超過30°C，記得攜帶雨具並注意防暑！"
                elif rain_prob_int > 30:
                    notification_message = "\n☔ 降雨機率超過30%，記得攜帶雨具！"
...
def schedule_weather_notifications():
    ...
    try:
        config = load_config()
        # 從配置中獲取天氣提醒時間
        weather_times = config.get('weather_times', {})
        weekday_default = weather_times.get('weekday_default', [])
        special_days = weather_times.get('special_days', {})
        # 如果今天是特殊提醒的日子，則使用特殊提醒時間
        notify_times = special_days.get(today, weekday_default)
        # 設定提醒時間
        ...
        weather_logger.info(f"今天是 {today}，設定的提醒時間為: {notify_times}")
    except (FileNotFoundError, ValueError) as e:
        weather_logger.error(f"配置文件錯誤: {e}")
        exit(1)
